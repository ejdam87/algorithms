...
def dfs_rec(graph: Graph, u: int, time: List[int]) -> None:

    time[0] += 1
    graph.discovery_time[u] = time[0]
    ...
    for v in range(graph.size):
        ...
        if is_edge(graph, u, v) and (not graph.visited[v]):
            ...
            graph.parent[v] = u
            dfs_rec(graph, v, time)

    time[0] += 1
    graph.finishing_time[u] = time[0]
...
def convert_to_matrix(succs: List[List[int]]) -> List[List[int]]:
    
    res = [[0 for i in range(len(succs))] for j in range(len(succs))]

    for u in range(len(succs)):
